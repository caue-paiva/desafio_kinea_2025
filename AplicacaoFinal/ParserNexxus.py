...
class ParserNexxus:
    ...
    def __agrega_reguas(self)->pd.DataFrame:
        ...
        for file in self.__PATH_REGUAS.glob("*.csv"): #todos os arquivos .csv
            nome_arquivo = str(file)
            ativo:str = nome_arquivo.split("_")[-1].split(".")[0] #pega string depois do _ e antes do .
            df = pd.read_csv(file)
            df["Product"] = ativo
            df_agregado = pd.concat([df_agregado, df])
         

        return df_agregado

    def regua_para_nexxus(self, nome_arquivo_finalizado:str = "input_nexxus_teste.csv")->None:
        """
        ...
        reguas_agregadas["financial_price"] = reguas_agregadas["preco"] * reguas_agregadas["quantidade"]
        ...
        reguas_agregadas = reguas_agregadas.rename({"ProductClass":"product_class"},axis=1)
...
if __name__ == "__main__":
    parser = ParserNexxus()
    parser.regua_para_nexxus("Oi.csv")